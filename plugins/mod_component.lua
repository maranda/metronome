...
-- ** Copyright (c) 2009-2013, Kim Alvefur, Marco Cirillo, Matthew Wild, Waqas Hussain
...
function module.add_host(module)
	if module:get_host_type() ~= "component" then
		error("Don't load mod_component manually, it should be for a component.", 0);
	end
				...
				if query.name == "query" and query.attr.xmlns == "http://jabber.org/protocol/disco#info" and (not node or node == "") then
					...
					if name then
						event.origin.send(st.reply(stanza):tag("query", { xmlns = "http://jabber.org/protocol/disco#info" })
							:tag("identity", { category = "component", type = "generic", name = module:get_option_string("name", "Metronome") }))
						return true;
					end
				end
			end
...
function stream_callbacks.streamopened(session, attr)
	if not module:host_is_component(attr.to) then
