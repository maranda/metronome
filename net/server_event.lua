...
--
-- This file is based on the unlicensed work of blastbeat created in date 2009/11/20
...
local cfg = {
	...
	READ_TIMEOUT = 20 * 60 * 6,  -- timeout in seconds for read data from socket
...
local newevent = (function()
	...
	return function(...) return add(base, ...); end
end)();

local closeallservers = function(arg)
	for _, item in ipairs(interfacelist()) do
		if item.type == "server" then item:close(arg) end
	end
end
