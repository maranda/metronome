...
function service:publish(node, actor, id, item, jid, _config)
	local node_obj = self.nodes[node];
	...
	if not node_obj and self.config.autocreate_on_publish then
		...
		local ok, err = self:create(node, true, _config, _actor);
		if not ok then
			return ok, err;
		end
		node_obj = self.nodes[node];
...
function service:get_items(node, actor, id, max, strip)
	...
	if actor ~= true
	   ...
	   and self:get_affiliation(actor, node) ~= "owner" then
		...
		if not is_whitelisted then return false, "forbidden"; end
	end
